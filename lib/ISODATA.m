...
for i = 1 : iteration
    ...
    % Whether the number of samples in each class is smaller than minimum_n
...
for i = 1 : centroid_x
    if((class_variance(i,1) > maximum_variance) && size(find(current_result == i),1) > (2*minimum_n))
        ...
        centroid(end+1,:) = centroid(i,:) - sqrt(maximum_variance); % Add one new class into the end of centroid
    end
end
end

% Combining in ISODATA
...
for i = 1 : x
    for j = 1 : x
        if((i ~= j) && (class_distance_matrix(i,j) <  minimum_d))
            ...
            centroid(j,:) = [];
            break; % Number of combine operation is limited to 1 through each loop
        end
    end
end
